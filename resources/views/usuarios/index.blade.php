            ...
            <div class="ibox ">
                <div class="ibox-content">
                    <div class="table-responsive">
                        <table class="table dataTables-usuarios table-striped table-bordered table-hover"
...
@include('usuarios.changePassword');
@stop
@push('styles')
<!-- DataTable -->
...
<script>
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        var html =
                            "<select id='tipo_documento_contacto' onchange='cambiorol(this)' data-user='" +
                            data.id +
                            "' name='tipo_documento_contacto' class='select2_form form-control'> ";
                        for (var i = 0; i < roles.length; i++) {
                            if (data.rol === roles[i].name) {
                                html = html + "<option value='" + roles[i].id + "' selected >" +
                                    roles[i].name + "</option>"
                            } else {
                                html = html + "<option  value='" + roles[i].id + "' >" + roles[
                                    i].name + "</option>"
                            }
                ...
                },
                {
                    data: null,
                    render: function($data) {
                        return `<button type="button" class="btn btn-danger changePassword" >Cambiar contraseña</button>`;
                    }
                }
            ],
            "language": {
    ...
    $(document).on("click", "#btn-actualizar-password", function(e) {

        try {
            if($("#newPassword").val().trim() =="")
            throw "Ingresa la nueva contraseña";
            $.ajax({
                dataType: 'json',
                type: 'POST',
